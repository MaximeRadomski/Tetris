...
public class MemorialCleaningBehavior : MonoBehaviour
{
	...
	void Start()
	{
		...
		while (i < 4)
		{
			numberLines[i] = Instantiate(scoreNumberSample, new Vector3(7.5f - (i + 1) * 1.2f, 11.5f, 0), transform.rotation) as GameObject;
			++i;
		}
		if (PlayerPrefs.GetInt("CleaningRecord") == 0)
			return;
		AddTo(numberLines, PlayerPrefs.GetInt("CleaningBestLines").ToString());
		Debug.Log("Best Time: "+ PlayerPrefs.GetFloat("CleaningBestTime"));
		int minutes = (int)(PlayerPrefs.GetFloat("CleaningBestTime") / 60);
		int seconds = (int)(PlayerPrefs.GetFloat("CleaningBestTime") % 60);
		int milliseconds = (int)((PlayerPrefs.GetFloat("CleaningBestTime") % 1) * 1000);
